...
1. **Import workflow**: Paste `007_error-handler.n8n.json` into n8n
2. **Configure credentials**:
   ...
   - Update `YOUR_TELEGRAM_CHAT_ID` in the Format Telegram Alert code node (or it's already set if using the version with real credentials)
...
- 🔴 Critical (auth, resource)
- 🟠 High (rate limit, network)
- 🟡 Medium (parse, validation, unknown)
...
Use a Code node with test errors (examples in the workflow's sticky note):
```js
